function [sol, norm_l1] = solver_prox_L1_par(x, lambda, param)
...
if param.tight && ~param.pos && ~param.real % TIGHT FRAME CASE
    ...
    dummy = [];
    for k = 1:P
        dummy_ = param.Psit{k}(sol);
        u_l1{k} = zeros(size(dummy_));
        dummy = [dummy; dummy_];
    end
    ...
    while 1
        ...
        for k = 1:P
            f(k) = parfeval(@run_par_waverec, 3, sol, param.Psit{k}, param.Psi{k}, u_l1{k}, param.nu, lambda, param.weights);
        end
        norm_l1 = 0;
        upd = zeros(size(x));
        for k = 1:P
            [idx, upd_, u_l1{k}, dummy] = fetchNext(f);
            % L1 norm of the estimate
            norm_l1 = norm_l1 + sum(param.weights(:).*abs(dummy(:)));
            upd = upd + upd_;
        end
...
function [upd, u_l1, dummy] = run_par_waverec(sol, Psit, Psi, u_l1, nu, lambda, weights)
    dwtmode('per');
    ...
    dummy = Psit(sol);
    res = u_l1*nu + dummy;
    dummy_ = soft(res, lambda*nu*weights);
    u_l1 = 1/nu * (res - dummy_);
    upd = Psi(u_l1);
end